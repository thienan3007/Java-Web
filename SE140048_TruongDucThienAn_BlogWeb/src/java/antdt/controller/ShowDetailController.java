...
public class ShowDetailController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ArticleDTO articleDTO = articleDAO.getArticle(Integer.parseInt(detailID));
            request.setAttribute("article", articleDTO);
            request.getRequestDispatcher("detailPage").forward(request, response);
        } catch (SQLException ex) {
            log("ShowDetailController _ SQL " + ex.getMessage());
        } catch (ClassNotFoundException ex) {
            log("ShowDetailController _ ClassNotFound " + ex.getMessage());
        }
    }
