 ...
 * @author antru
 */
@WebServlet(name = "ActiveArticleController", urlPatterns = {"/ActiveArticleController"})
        ...
        try {
            if (accountDTO == null) {
                ...
                if (accountDTO.getRole().getId() == 1) {
                    ...
                    if (articleDAO.activeArticle(Integer.parseInt(articleID))) {
                        request.getRequestDispatcher("homePageControl").forward(request, response);
                    }
                } else {
                    request.getRequestDispatcher("homePageControl").forward(request, response);
            ...
            log("ActiveArticleController _ SQl " + ex.getMessage());
        } catch (ClassNotFoundException ex) {
            log("ActiveArticleController _ ClassNotFound " + ex.getMessage());
